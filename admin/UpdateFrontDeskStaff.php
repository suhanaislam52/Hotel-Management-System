...
$first_name = $last_name = $phone_number = $gender = $address = $salary = $in_charge_of = "";
$staff_id = 0;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['select_staff'])) {
    ...
    $sql = "SELECT * FROM `frontdeskstaff` WHERE first_name = ? AND last_name = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_staff'])) {
    ...
    $sql = "UPDATE `frontdeskstaff` SET first_name = ?, last_name = ?, Phone_number = ?, Gender = ?, Address = ?, Salary = ?, In_Charge_of = ? WHERE ID = ?";
    $values = array($first_name, $last_name, $phone_number, $gender, $address, $salary, $in_charge_of, $staff_id);
    $datatypes = "sssssssi";
...
<head>
    ...
    <title>Update Front Desk Staff</title>
    <style>
        body {
...
<body>
    <h2 style="text-align: center;">Update Front Desk Staff</h2>
